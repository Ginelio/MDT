# when [] is reference from text Georgia_Karataraki.pdf
# comments denoted with ## come from the reviewer

# This file stays true to the original equations of the main paper [http://dx.doi.org/10.1016/j.enconman.2014.07.078]
...
# Wind Data derived from the wind data analysis file
...
# Hydrogen production cost [Equation 12]
def C_H2_CH(P_EL, Y_EL, Y_FC, parameters):
    C_H2_CH = ((parameters['C_EL_IN'] / parameters['L_EL']) + ((Y_EL * parameters['C_O_and_M_EL'])))/ (parameters['η_EL'] * parameters['η_FC']) + ((parameters['C_FC_IN'] / parameters['L_FC']) + ((Y_FC * parameters['C_O_and_M_FC']) * (parameters['P_EL_REF'] / parameters['P_NOM_FC']))) / (parameters['η_EL'] * parameters['η_FC'])
    return C_H2_CH
...
def C_EX(P_EX, parameters):
    C_EX = parameters['C_EX'] * P_EX
    return C_EX

# Undelivered electricity cost
def C_UN(P_UN, parameters):
    C_UN = parameters['C_UN'] * P_UN
    return C_UN
...
def objective_function(P_B_CH, P_B_DIS, Y_B_CH, Y_B_DIS, Y_EL, Y_FC, P_EX, P_UN):
    obj = gp.LinExpr()
    for t in range(num_periods):
        ...
        C_EX_val = C_EX(P_EX[t], parameters)
        C_UN_val = C_UN(P_UN[t], parameters)
...
# Objective function
model.setObjective(objective_function(P_B_CH, P_B_DIS, Y_B_CH, Y_B_DIS, Y_EL, Y_FC, P_EX, P_UN), gp.GRB.MINIMIZE)
...
if model.status == gp.GRB.Status.OPTIMAL:
    ...
    for t in range(num_periods):
        ...
        results['C_EX'].append(C_EX(P_EX[t], parameters).getValue())
        results['C_UN'].append(C_UN(P_UN[t], parameters).getValue())
        results['Total Cost'].append((C_B_CH(P_B_CH[t], Y_B_CH[t], parameters) + 
                                      ...
                                      C_EX(P_EX[t], parameters) + 
                                      C_UN(P_UN[t], parameters)).getValue())
else:
